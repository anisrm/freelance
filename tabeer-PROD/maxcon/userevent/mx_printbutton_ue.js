...
define(['N/runtime', 'N/search', 'N/record', 'N/format'], function (runtime, search, record, format) {
    ...
    function anotherLineItems(context) {
        try {
            ...
            var date1    = rec1.getValue("trandate");
            var trandate_1   = format.format({value:date1, type: format.Type.DATE})
            log.debug('trandate_1', trandate_1);
            log.debug('date_1', format.format({value: new Date(), type: format.Type.DATE}));
    ...
    function getLineItems(context) {
        try {
            ...
            var tableItems = '<table width="100%" table-layout="fixed">'
                ...
                for (var i = 1; i <= values.length; i++) {
                   ...
                   /*  tableItems += '<tr >'
                    ...
                    if (description.length > 500) {
                        ...
                        tableItems += '<td  align="right" colspan="4">' + formatAmount(amount) + '</td>'
                        tableItems += '<td align="left" colspan="6"><p align="center">' + locations + '</p></td>'
                        tableItems += '</tr>'
